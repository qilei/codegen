...
<#assign type="com.dstz."+system+package+".model." +class>
...
<mapper namespace="com.dstz.${system}${package}.dao.${class}Dao">
	...
	<insert id="bulkCreate" parameterType="${type}" databaseId="mysql">
		INSERT INTO ${tableName}
		(<#list colList as col>${col.columnName}<#if col_has_next>,</#if></#list>)
		VALUES 
		<foreach collection="list" item="item" index="index" separator=","> 
			(<#list colList as col><#assign colName=func.convertUnderLine(col.columnName)><#noparse>#{</#noparse>item.${colName},jdbcType=${func.getJdbcType(col.colDbType)}<#noparse>}</#noparse><#if col_has_next>, </#if></#list>)
		</foreach>
	</insert>
	...
	<delete id="bulkRemove" parameterType="java.lang.String">
		DELETE FROM ${tableName}
		WHERE id_ in 
		<foreach collection="array" item="id" index="index" open="(" close=")" separator=",">  	<#noparse>#{</#noparse>id}  </foreach>
	</delete>
